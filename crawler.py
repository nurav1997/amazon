...
def telegram_alert(msg_content):
    ...
    if msg_content['code'] == 1:
        message = "🥳 PRICE DROPPED ! TO ₹" + \
            str(msg_content['Price']) + "  FOR" + \
            msg_content['Product'] + " " + " " + msg_content['URL']
...
def get_price(url, selector):
    ...
    try:
        ...
        price_string = re.findall(
            '\d+.\d+', tree.xpath(selector['price'])[0].text)[0]
...
def main():
    ...
    while True and len(items):
        ...
        for item in copy(items):
            # url to parse
            item_page_url = urllib.parse.urljoin(
                config['amazon-base_url'], item[0])
            print('[#%02d] Checking price for %s (target price: %s)' %
                  (itemIndex, item[0], item[1]))

            # get price and product name
            productName = item[2]
            price, productName = get_price(
                item_page_url, config['xpath_selector'])
            ...
            if not price:
                ...
                print('[#%02d] %s\'s price is %s. Ignoring...' %
                      (itemIndex, productName, price))
            itemIndex += 1
        ...
        if len(items):
            ...
            nowtime = datetime.now()
            thisIntervalTime = intervalTimeBetweenCheck + \
